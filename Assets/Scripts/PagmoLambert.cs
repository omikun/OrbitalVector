...
/*
   ...
   problem through the variable X=Math.Log(1+cos(alpha/2)). By doing so the
...
{ 
    ...
    //FIXME
    double x2tof(double a, double b, double c, bool d)
    {
        return a + b;
    }
void LambertI (ref double[] r1_in, ref double[] r2_in, double t, double mu, //INPUT
		ref bool lw, //INPUT
		ref double[] v1, ref double[] v2, ref double a, ref double p, ref double theta, ref int iter)//OUTPUT
{
	double V,T,
	...
	if (t <= 0)
	{
		...
		Debug.Assert(false, "error in lambert solver: negative time in input");
	}
	...
	// We start finding the Math.Log(x+1) value of the solution conic:
	// NO MULTI REV --> (1 SOL)
	...
	// The solution has been evaluated in terms of Math.Log(x+1) or tan(x*pi/2), we
	...
	//FIXME vett(r1,r2,ih_dum);
	//FIXME vers(ih_dum,ih) ;
...
//FIXME 	vers(r2,r2_vers);
//FIXME 	vett(ih,r2_vers,dum);
	for (i = 0;i < 3 ;i++)
